...
public class VerifyDependencyCallViewModelTests_Test {
  ...
  public void then_call_Logger_Log___() {
    ...
    for (int callInfoIndex = 0; callInfoIndex <= spy.LogCallInfos.size(); callInfoIndex++) {
      ...
      Assert.assertEquals("Logger.Log Call[" + Integer.toString(callInfoIndex) + "]: Expected argument value 'message' is <" + "my log" + "> but was <" + callInfo.messageValue + ">", "my log", callInfo.messageValue);
    }
    Assert.assertEquals("Expected that Logger.Log was called exactly 1 times, but was " + Integer.toString(spy.LogCallInfos.size()), Integer.valueOf(1), Integer.valueOf(spy.LogCallInfos.size()));
  }
  public void then_call_Logger_Log____1() {
    ...
    if (3 < spy.LogCallInfos.size()) {
      ...
      Assert.assertEquals("Logger.Log Call[" + Integer.toString(3) + "]: Expected argument value 'message' is <" + "my log" + "> but was <" + callInfo.messageValue + ">", "my log", callInfo.messageValue);
    } else {
      Assert.assertTrue("Expected a call to Logger.Log at index [3], but it was only called " + Integer.toString(spy.LogCallInfos.size()) + " times", false);
    }
  }
  public void then_call_Logger_Log____2() {
    ...
    Assert.assertTrue("Expected that Logger.Log was called at least 3 times, but was " + Integer.toString(spy.LogCallInfos.size()), spy.LogCallInfos.size() >= 3);
    Assert.assertTrue("Expected that Logger.Log was called at most 6 times, but was " + Integer.toString(spy.LogCallInfos.size()), spy.LogCallInfos.size() <= 6);
  }
}

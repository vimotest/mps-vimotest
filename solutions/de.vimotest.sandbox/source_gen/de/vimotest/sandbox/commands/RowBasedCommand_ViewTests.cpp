...
namespace commands
{
  ...
  void RowBasedCommand_ViewTests::then_MyFlags_has_2_rows()
  {
    ...
    EXPECT_EQ(2, actualRows.size()) << std::string("Expected that list view MyFlags has 2 rows, but has ") + actualRows.size();
    // {
    auto& row0 = actualRows.at(0);
    EXPECT_EQ(0, row0->getRowIndex()) << std::string("Expected that list view MyFlags row at index 0 has rowhandle <0>, but was <") + row0->getRowIndex() + std::string(">");
    ;
    // }
    // {
    auto& row1 = actualRows.at(1);
    EXPECT_EQ(1, row1->getRowIndex()) << std::string("Expected that list view MyFlags row at index 1 has rowhandle <1>, but was <") + row1->getRowIndex() + std::string(">");
    ;
    // }
  }
  void RowBasedCommand_ViewTests::then_MyComboBoxes_has_2_rows()
  {
    ...
    EXPECT_EQ(2, actualRows.size()) << std::string("Expected that table view MyComboBoxes has 2 rows, but has ") + actualRows.size();
    // {
    auto& row0 = actualRows.at(0);
    EXPECT_EQ(0, row0->getRowIndex()) << std::string("Expected that table view MyComboBoxes row at index 0 has rowhandle <0>, but was <") + row0->getRowIndex() + std::string(">");
    auto& actualRow0FreeTextComboBoxesEntries = row0->getFreeTextComboBoxesComboBoxEntries();
    EXPECT_EQ(0, actualRow0FreeTextComboBoxesEntries.size()) << std::string("Expected that combobox FreeTextComboBoxes has 0 entries, but has ") + actualRow0FreeTextComboBoxesEntries.size();
    EXPECT_EQ(nullptr, row0->getFreeTextComboBoxesComboBoxSelectedEntry()) << std::string("Expected that combobox FreeTextComboBoxes has ") + std::string("no selection") + std::string(", but was <") + row0->getFreeTextComboBoxesComboBoxSelectedEntry() + std::string(">");
    ...
    EXPECT_EQ(3, actualRow0EntryComboBoxesEntries.size()) << std::string("Expected that combobox EntryComboBoxes has 3 entries, but has ") + actualRow0EntryComboBoxesEntries.size();
    ...
    EXPECT_EQ(nullptr, row0->getEntryComboBoxesComboBoxSelectedEntry()) << std::string("Expected that combobox EntryComboBoxes has ") + std::string("no selection") + std::string(", but was <") + row0->getEntryComboBoxesComboBoxSelectedEntry() + std::string(">");
    // }
    // {
    auto& row1 = actualRows.at(1);
    EXPECT_EQ(1, row1->getRowIndex()) << std::string("Expected that table view MyComboBoxes row at index 1 has rowhandle <1>, but was <") + row1->getRowIndex() + std::string(">");
    auto& actualRow1FreeTextComboBoxesEntries = row1->getFreeTextComboBoxesComboBoxEntries();
    EXPECT_EQ(0, actualRow1FreeTextComboBoxesEntries.size()) << std::string("Expected that combobox FreeTextComboBoxes has 0 entries, but has ") + actualRow1FreeTextComboBoxesEntries.size();
    EXPECT_EQ(nullptr, row1->getFreeTextComboBoxesComboBoxSelectedEntry()) << std::string("Expected that combobox FreeTextComboBoxes has ") + std::string("no selection") + std::string(", but was <") + row1->getFreeTextComboBoxesComboBoxSelectedEntry() + std::string(">");
    ...
    EXPECT_EQ(3, actualRow1EntryComboBoxesEntries.size()) << std::string("Expected that combobox EntryComboBoxes has 3 entries, but has ") + actualRow1EntryComboBoxesEntries.size();
    ...
    EXPECT_EQ(nullptr, row1->getEntryComboBoxesComboBoxSelectedEntry()) << std::string("Expected that combobox EntryComboBoxes has ") + std::string("no selection") + std::string(", but was <") + row1->getEntryComboBoxesComboBoxSelectedEntry() + std::string(">");
    // }
  }
  void RowBasedCommand_ViewTests::then_MyStandardControls_has_2_rows()
  {
    auto& actualRows = this->sut->getMyStandardControlsTreeRows();
    EXPECT_EQ(2, actualRows.size()) << std::string("Expected that tree view MyStandardControls has 2 rows, but has ") + actualRows.size();
    // {
    auto& row0 = actualRows.at(0);
    EXPECT_EQ(0, row0->getRowIndex()) << std::string("Expected that tree view MyStandardControls row at index 0 has rowhandle <0>, but was <") + row0->getRowIndex() + std::string(">");
    EXPECT_EQ(0, row0->getRowDepth()) << std::string("Expected that tree view MyStandardControls row at index 0 has parent rowhandle <") + 0 + std::string(">, but was <") + row0->getRowDepth() + std::string(">");
    ...
    // {
    auto& row1 = actualRows.at(1);
    EXPECT_EQ(1, row1->getRowIndex()) << std::string("Expected that tree view MyStandardControls row at index 1 has rowhandle <1>, but was <") + row1->getRowIndex() + std::string(">");
    EXPECT_EQ(0, row1->getRowDepth()) << std::string("Expected that tree view MyStandardControls row at index 1 has parent rowhandle <") + 0 + std::string(">, but was <") + row1->getRowDepth() + std::string(">");

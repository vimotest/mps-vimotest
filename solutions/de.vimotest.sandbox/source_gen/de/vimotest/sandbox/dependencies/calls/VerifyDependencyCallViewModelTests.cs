...
public class VerifyDependencyCallViewModelTests
{
    ...
    public virtual void then_call_Logger_Log_any_()
    {
        ...
        Assert.AreEqual(1, spy.LogCallInfos.Count, "Expected that Logger.Log was called exactly 1 times, but was " + IntegerFunctions.ToString(spy.LogCallInfos.Count));
    }
    ...
    public virtual void then_call_Logger_Log___()
    {
        ...
        for (int callInfoIndex = 0; callInfoIndex <= spy.LogCallInfos.Count; callInfoIndex++)
        {
            ...
            Assert.AreEqual("my log", callInfo.messageValue, "Logger.Log Call[" + IntegerFunctions.ToString(callInfoIndex) + "]: Expected argument value 'message' is <" + "my log" + "> but was <" + callInfo.messageValue + ">");
        }
        Assert.AreEqual(1, spy.LogCallInfos.Count, "Expected that Logger.Log was called exactly 1 times, but was " + IntegerFunctions.ToString(spy.LogCallInfos.Count));
    }
    ...
    public virtual void then_call_Logger_Log____1()
    {
        ...
        if (3 < spy.LogCallInfos.Count)
        {
            ...
            Assert.AreEqual("my log", callInfo.messageValue, "Logger.Log Call[" + IntegerFunctions.ToString(3) + "]: Expected argument value 'message' is <" + "my log" + "> but was <" + callInfo.messageValue + ">");
        }
        else
        {
            Assert.IsTrue(false, "Expected a call to Logger.Log at index [3], but it was only called " + IntegerFunctions.ToString(spy.LogCallInfos.Count) + " times");
        }
    }
    ...
    public virtual void then_call_Logger_Log____2()
    {
        ...
        Assert.IsTrue(spy.LogCallInfos.Count >= 3, "Expected that Logger.Log was called at least 3 times, but was " + IntegerFunctions.ToString(spy.LogCallInfos.Count));
        Assert.IsTrue(spy.LogCallInfos.Count <= 6, "Expected that Logger.Log was called at most 6 times, but was " + IntegerFunctions.ToString(spy.LogCallInfos.Count));
    }
}

...
namespace widgetassertions
{
  ...
  void TableViews_ViewTest::then_MyStringRowHandles_has_2_rows_and_selected_row_handle_ROW1()
  {
    ...
    // {
    auto& row1 = actualRows.at(1);
    EXPECT_EQ(std::string("ROW1"), row1->getRowHandle()) << std::string("Expected that table view MyStringRowHandles row at index 1 has rowhandle <ROW1>, but was <") + row1->getRowHandle() + std::string(">");
    ...
    // }
    EXPECT_EQ(std::string("ROW1"), this->sut->getMyStringRowHandlesTableSelectedRow()) << std::string("Expected that table view MyStringRowHandles has selected row with row handle <") + std::string("ROW1") + std::string(">, but was <") + this->sut->getMyStringRowHandlesTableSelectedRow() + std::string(">");
  }

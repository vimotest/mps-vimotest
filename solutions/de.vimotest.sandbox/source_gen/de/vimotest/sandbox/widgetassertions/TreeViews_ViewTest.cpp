...
namespace widgetassertions
{
  ...
  void TreeViews_ViewTest::then_MyParentStringRowHandles_has_4_rows()
  {
    ...
    EXPECT_EQ(std::string("-"), row0->getParentRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 0 has parent rowhandle <") + std::string("-") + std::string(">, but was <") + row0->getParentRowHandle() + std::string(">");
    EXPECT_EQ(std::string("A"), row0->getValuesLabelText()) << std::string("Expected that label Values has text <") + std::string("A") + std::string(">, but was <") + row0->getValuesLabelText() + std::string(">");
    // }
    // {
    auto& row1 = actualRows.at(1);
    EXPECT_EQ(std::string("HANDLE_B"), row1->getRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 1 has rowhandle <HANDLE_B>, but was <") + row1->getRowHandle() + std::string(">");
    EXPECT_EQ(std::string("HANDLE_A"), row1->getParentRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 1 has parent rowhandle <") + std::string("HANDLE_A") + std::string(">, but was <") + row1->getParentRowHandle() + std::string(">");
    EXPECT_EQ(std::string("A.B"), row1->getValuesLabelText()) << std::string("Expected that label Values has text <") + std::string("A.B") + std::string(">, but was <") + row1->getValuesLabelText() + std::string(">");
    // }
    // {
    auto& row2 = actualRows.at(2);
    EXPECT_EQ(std::string("HANDLE_C"), row2->getRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 2 has rowhandle <HANDLE_C>, but was <") + row2->getRowHandle() + std::string(">");
    EXPECT_EQ(std::string("-"), row2->getParentRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 2 has parent rowhandle <") + std::string("-") + std::string(">, but was <") + row2->getParentRowHandle() + std::string(">");
    EXPECT_EQ(std::string("C"), row2->getValuesLabelText()) << std::string("Expected that label Values has text <") + std::string("C") + std::string(">, but was <") + row2->getValuesLabelText() + std::string(">");
    // }
    // {
    auto& row3 = actualRows.at(3);
    EXPECT_EQ(std::string("HANDLE_D"), row3->getRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 3 has rowhandle <HANDLE_D>, but was <") + row3->getRowHandle() + std::string(">");
    EXPECT_EQ(std::string("HANDLE_C"), row3->getParentRowHandle()) << std::string("Expected that tree view MyParentStringRowHandles row at index 3 has parent rowhandle <") + std::string("HANDLE_C") + std::string(">, but was <") + row3->getParentRowHandle() + std::string(">");
    EXPECT_EQ(std::string("C.D"), row3->getValuesLabelText()) << std::string("Expected that label Values has text <") + std::string("C.D") + std::string(">, but was <") + row3->getValuesLabelText() + std::string(">");
    // }
  }
